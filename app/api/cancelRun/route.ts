...
export async function POST(req: NextRequest) {
  try {
    ...
    const canceledRun = await openai.beta.threads.runs.cancel(threadId, runId);

    // Log the retrieved run status for debugging
    console.log(`Canceled run: ${canceledRun}`);

    // Return the retrieved run status as a JSON response
    return NextResponse.json({ canceledRun });
  } catch (error) {
