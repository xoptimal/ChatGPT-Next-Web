...
export const cozeBots = async () => {

  const response = await fetch(`/api/cozeBots`, {
    method: "GET",
  });
...
// Cancels run
export const cancelOngoingRun = async (
...
export const listRuns = async (threadId: string | null) => {
  ...
  if (!response.ok) {
    console.error(
      `Error fetching runs: ${response.status} ${response.statusText}`,
    );
    throw new Error(
      `Failed to list runs: ${response.status} ${response.statusText}`,
    );
  }
  const jsonResponse = await response.json();
...
export const getAssistantResponse = async (threadId: string, runId: string) => {
  ...
  if (!response.ok) {
    console.error(
      `Error fetching assistant response: ${response.status} ${response.statusText}`,
    );
    throw new Error(
      `Failed to fetch assistant response: ${response.status} ${response.statusText}`,
    );
  }
  const jsonResponse = await response.json();
