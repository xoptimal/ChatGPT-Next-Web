...
export const useAssistantStore = createPersistStore(
    DEFAULT_CHAT_STATE,
    (set, _get) => {
        function get() {
            return {
        ...
        const methods = {
            ...
            async onUserInput(content: string) {
                ...
                if (!session.threadId) {
                    ...
                    // update
                    get().updateCurrentSession((session) => (session.threadId = threadId));
                }
                ...
                try {
                    //  重新请求
                    const runId = await runChatAssistant(session.id, session.threadId);
